...
# ── Batch Size ────────────────────────────────────────────────────────────────
# For Intel Iris Xe shared memory, use small batch size
GPU_BATCH_SIZE = 1  # Process one frame at a time for shared memory

# ── RAM Safety ────────────────────────────────────────────────────────────────
RAM_LIMIT_PCT = 70  # Reduced to leave more headroom
...
class ProcessingPipeline:
    ...
    -----------------------------------------------------
    Uses NexusNet AI model for upscaling
    PNG format for lossless quality
    Supports image enhancement (brightness, sharpness, contrast, saturation)
    """
    ...
    def __init__(self):
        ...
        self.video_info = None  # Store video info for later use
    ...
    def _run(self, input_path, output_path, settings):
        try:
            ...
            enhance = settings.get("enhance", False)  # Image enhancement setting
            interpolate = settings.get("interpolate", True)
            target_fps = settings.get("target_fps", 60.0)
            model_path = settings.get("model_path", "model_custom/nexusnet_best.pth")
            ...
            # Initialize AI upscaler with image enhancement
            ...
            # Store video info for UI
            self.video_info = info

            self._stage(
                self.STAGE_ANALYSIS, "done",
                f"{src_w}x{src_h} @ {src_fps:.2f}fps | "
                f"{duration:.1f}s | "
                f"{'VFR' if is_vfr else 'CFR'} | "
                f"{self._format_time(int(time.time()-t0))}"
            )
            self._status(
                f"Source : {src_w}x{src_h} @ "
                f"{src_fps:.2f}fps | {duration:.1f}s | "
                f"{'VFR' if is_vfr else 'CFR'}"
            )
            ...
            if is_vfr:
                self._stage(
                    self.STAGE_VFR_CFR, "active",
                    f"Converting VFR to CFR @ {src_fps:.2f}fps..."
                )
                self._status(f"VFR detected - converting to CFR...")
                t0 = time.time()
                process_path = chunker.convert_vfr_to_cfr(
                    input_path, src_fps,
                    status_callback=self._status
                )
                elapsed = self._format_time(int(time.time() - t0))
                self._stage(
                    self.STAGE_VFR_CFR, "done",
                    f"CFR @ {src_fps:.2f}fps | {elapsed}"
                )
                self._status(f"✓ VFR → CFR conversion complete")
            else:
                self._stage(
                    self.STAGE_VFR_CFR, "done",
                    "Already CFR - no conversion needed"
                )
            ...
            interp_steps, out_fps, interp_method = \
                upscaler.calculate_interpolation_steps(
                    src_fps, target_fps
                )
            ...
            if not skip_upscale:
                self._status(
                    f"Upscale : {src_w}x{src_h} → {out_w}x{out_h} (AI Model)"
                )
            if do_interpolate:
                self._status(
                    f"Interpolation : {src_fps:.2f} → {out_fps:.0f}fps [{interp_method}]"
                )

            self._status(f"Engine : NexusNet AI Model")
            self._status(f"Format : PNG (Lossless)")
            if enhance:
                self._status(f"Image Enhancement : ON (Brightness -5%, Sharpness +30%, Contrast +15%, Saturation +5-15%)")
            ...
            self._status(
                f"Split into {total_chunks} chunk(s) · "
                f"~{int(src_fps * 60)} frames/chunk"
            )
            ...
            for chunk in chunks:
                ...
                self._stage(
                    self.STAGE_EXTRACTION, "active",
                    f"Chunk {idx+1} — Extracting ~{expected} frames..."
                )
                ...
                self._stage(
                    self.STAGE_EXTRACTION, "done",
                    f"Chunk {idx+1} — {total_frames} frames extracted | {elapsed}"
                )
                self._status(f"  ✓ Extracted {total_frames} frames")

                out_frames_dir.mkdir(parents=True, exist_ok=True)

                # ── Stage 5: AI Upscaling ─────────────────────────────────
                self._stage(
                    self.STAGE_UPSCALING, "active",
                    f"Chunk {idx+1} — Upscaling {total_frames} frames..."
                )
                ...
                for batch_start in range(0, total_frames, batch_size):
                    ...
                    if sample_src_frame is None:
                        sample_src_frame = batch_frames[0].copy()

                    # Process entire batch with AI model
                    if skip_upscale:
                        processed = [upscaler.enhance_only(f) for f in batch_frames]
                    else:
                        processed = upscaler.process_batch(batch_frames, src_w, src_h)

                    # Capture sample output
                    if sample_out_frame is None and processed:
                        sample_out_frame = processed[0].copy()

                    # Write results to disk (PNG format)
                ...
                self._stage(
                    self.STAGE_UPSCALING, "done",
                    f"Chunk {idx+1} — {out_frame_idx} frames upscaled | {upscale_elapsed}"
                )
                ...
                # ── Stage 6: Rebuild ──────────────────────────────────────
                self._stage(
                    self.STAGE_REBUILD, "active",
                    f"Chunk {idx+1} — Encoding {out_frame_idx} frames..."
                )
                self._status(f"  Encoding chunk {idx+1}...")
                t0 = time.time()
                chunker.rebuild_chunk_video(
                    out_frames_dir,
                    chunk_output,
                    out_fps,
                    frame_ext="png"
                ...
                elapsed = self._format_time(int(time.time() - t0))
                self._stage(
                    self.STAGE_REBUILD, "done",
                    f"Chunk {idx+1} encoded | {elapsed}"
                )
                ...
                self._status(
                    f"  ✓ Chunk {idx+1}/{total_chunks} complete "
                    f"[{self._format_time(int(time.time()-start_time))} elapsed]"
                )

            # ── Stage 7: Assembly ─────────────────────────────────────────
            self._stage(
                self.STAGE_ASSEMBLY, "active",
            ...
            audio_arg = (
                str(audio_path)
                if info["has_audio"] and audio_path.exists()
                else None
            )
            chunker.concatenate_chunks(
                completed_chunks, output_path, audio_arg
            )
            elapsed = self._format_time(int(time.time() - t0))
            self._stage(
                self.STAGE_ASSEMBLY, "done",
                f"Assembled | {elapsed}"
            )
